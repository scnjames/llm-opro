...
def download_file(
    ...
    if os.path.exists(target_path):
        ...
        LOGGER.info(
            f"Not downloading {source_uri} because {target_path} already exists"
        )
        return
    ...
    if source_uri.startswith("s3://"):
        ...
        downloader_executable: str = (
            "gdown" if source_uri.startswith("https://drive.google.com") else "wget"
        )
    ...
    if unpack:
        ...
        if unpack_type == "untar":
            ...
            with open(tmp_path, "rb") as ifh, open(
                os.path.join(tmp2_path, "data"), "wb"
            ) as ofh:
