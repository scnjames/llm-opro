...
def generate_answers(demo_examples, test_examples, prompt_candidate):
    ...
    pool = ThreadPool(
        processes=THREADS
    )  # allows us to run tasks in parallel
    candidate_problem_prompts = [
        (build_problem_prompt(p, demo_examples, prompt_candidate))
        for p in test_examples
    ]
    answers = []

    async_results = pool.map(
        generate_answer_proc, candidate_problem_prompts
    )  # This will apply the function to each prompt in parallel using as many cores as are available in the system
...
def get_prompt_candidate_score(generated_answers, problem_examples):
    ...
    ground_truth_answers = [
        p.answer.split(FINAL_ANSWER_SEP)[-1].strip().replace(",", "")
        for p in problem_examples
    ]
    return sum(
        [1 if a == b else 0 for a, b in zip(generated_answers, ground_truth_answers)]
...
def score_prompt_candidates(
    ...
    """Function to generate score of prompts on test set."""
    ...
    for prompt_candidate in tqdm(prompt_candidates, desc="Scoring prompt candidates"):
        ...
        scored_prompt_candidates.append(
            PromptExample(prompt=prompt_candidate, score=prompt_candidate_score)
        )
