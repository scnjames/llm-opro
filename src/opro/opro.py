...
def seed_prompt_examples(
    demo_examples: List[ProblemExample], test_examples: List[ProblemExample]
) -> List[PromptExample]:
    default_prompt_examples = ["Let’s solve the problem."]
    return score_prompt_candidates(
        default_prompt_examples, demo_examples, test_examples
    )


def update_prompt_examples(
    previous_prompt_examples: List[PromptExample],
    scored_prompt_candidates: List[PromptExample],
) -> List[PromptExample]:
...
def main():
    ...
    for _ in tqdm(range(MAX_ITER), desc="Optimization iteration"):
        ...
        scored_prompt_candidates = score_prompt_candidates(
            prompt_candidates, train_examples, test_examples
        )
        prompt_examples = update_prompt_examples(
            prompt_examples, scored_prompt_candidates
        )
